...
try{
 ...
 const page=await context.newPage();await page.goto(`chrome-extension://${id}/sidepanel/panel.html`); await page.waitForFunction(()=>document.querySelector('#footerVersion').textContent==='v2.2.0');
 ...
 await poster.setContent(`<style>*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:#142c46;color:white}main{padding:${small?'28px':'65px 90px'};height:${height}px;background:radial-gradient(ellipse at 90% 10%,#254c80,transparent 65%)}.brand{font-size:${small?21:30}px;font-weight:bold}.brand b{background:#eef4ff;color:#173451;padding:8px;border-radius:10px;margin-right:9px}h1{font-size:${small?34:74}px;line-height:1.08;letter-spacing:-1px;max-width:960px;margin:${small?'26px':'48px'} 0 18px}h1 span{color:#8ab3ff}p{font-size:${small?13:23}px;color:#bdcce1}footer{margin-top:${small?22:38}px;font-size:${small?11:15}px;color:#bacce1}</style><main><div class="brand"><b>D.</b>Diggio Agent IA</div><h1>Dal pensiero<br><span>all’azione, nel browser.</span></h1><p>Il tuo modello. Chat, agente e procedure Insegnami.</p><footer>Diggio3000 · www.diggio3000.it</footer></main>`);await poster.evaluate(async()=>{await Promise.all([...document.images].map(i=>i.decode()));await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));});
